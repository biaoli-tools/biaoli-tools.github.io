...
function promoBanner() {
  return `<section class="promo-band" aria-label="合作推广"><div class="shell"><a class="promo-banner" href="https://huyuejsq.co/" target="_blank" rel="sponsored noopener noreferrer"><span class="promo-label">推广</span><span class="promo-copy"><strong>虎跃加速</strong><span>多平台网络连接工具，查看产品介绍与下载方式</span></span><span class="promo-action">访问官网 <span aria-hidden="true">↗</span></span></a></div></section>`;
}
...
function policyPage(kind) {
  const pages = {
    ...
    privacy: { title: "隐私政策", description: "说明表理工具如何在浏览器本地处理文件，以及访问统计和推广链接的隐私边界。", html: `<p>更新日期：2026 年 9 月 9 日</p><h2>文件处理</h2><p>表理工具的表格解析、整理和导出默认在你的浏览器中完成。本站不提供文件上传接口，也不主动保存文件名、表头、单元格内容或导出结果。</p><h2>本地临时数据</h2><p>处理状态只存在于当前页面内存中。刷新或关闭页面后会清空。下载的结果由浏览器保存到你选择的位置。</p><h2>访问统计</h2><p>本站使用 Google Analytics 4 了解页面访问量、会话、访问来源以及浏览器和设备类别。Google Analytics 可能通过第一方 Cookie 区分用户与会话，并根据 IP 地址生成粗略地区信息。本站不会主动把文件名、列名、单元格内容或导出结果发送给统计服务。你可以通过浏览器设置限制或删除 Cookie；更多处理方式见 <a href="https://policies.google.com/privacy" rel="noopener noreferrer">Google 隐私权政策</a>。</p><h2>推广链接</h2><p>本站当前展示一处明确标注“推广”的虎跃加速横幅。横幅由本站 HTML 和 CSS 直接显示，不加载对方图片或广告脚本，也不设置额外的广告 Cookie。点击横幅后会离开本站，后续数据处理以对方网站的隐私政策为准。</p><h2>你需要留意的设备环境</h2><p>浏览器扩展、操作系统和下载目录不受本站控制。请勿在公共或不受信任的设备上处理敏感资料，使用后也要检查下载目录中是否留有副本。</p>` },
    terms: { title: "使用条款", description: "表理工具的使用范围、文件兼容限制、推广链接和责任边界。", html: `<p>更新日期：2026 年 9 月 9 日</p><h2>工具用途</h2><p>本站提供免费的浏览器端表格整理功能。你应确保有权处理所选择的文件，并对下载结果进行必要核对。</p><h2>兼容边界</h2><p>当前版本支持 XLSX、CSV 和 TSV，不处理旧版 XLS、加密或损坏的工作簿。日期会整理为统一格式，公式保存为公式文本；宏、图表、数据透视表、外部链接和复杂样式不会保留。</p><h2>重要数据</h2><p>处理前请保留原文件。财务结算、法律材料或其他高风险用途需要人工复核，不能只依赖自动生成的结果。</p><h2>推广链接</h2><p>标注“推广”的横幅会跳转到第三方网站。本站不对第三方产品的速度、安全性、可用性或适用性作保证；购买或使用前请自行核对对方的服务条款、隐私政策和退款规则。</p><h2>开源组件</h2><p>本站使用的开源组件及许可见<a href="/licenses/">开源许可页面</a>。</p>` },
